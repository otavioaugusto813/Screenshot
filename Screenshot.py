...
class PrintScreen:
    def __init__(self):
        ...
        escolher_pasta = tk.Button(app2, text='Escolher pasta', command=self.submit, borderwidth=3)

        escolher_pasta.grid(row=25, column=0)

        # TERCEIRA PARTE - EXECUÇÃO DOS PRINTS (INICIAR E INTERROMPER)

        app3 = tk.LabelFrame(self.root, text='Execução')
        botao_start = tk.Button(app3, text="Iniciar!", bg="light green",
                                command=self.start, width=20, height=4, borderwidth=3)
        botao_interromper = tk.Button(app3, text="Interromper", bg='red',
                                      command=self.stop, width=20, height=4, borderwidth=3)
        ...
        botao_apagar = tk.Button(app5, text='Zerar', command=self.apagar)
        botao_apagar.grid()
    ...
    def intervalo(self, evento):
        ...
        teste = self.isfloat(tempo)
        if teste:
            self.intervalo_tempo = float(tempo)
            self.intervalo_tempo = int(self.intervalo_tempo * 1000)
    ...
    def atrasar_inicio(self):
        ...
        self.root.after(2000, self.tirar_print)
    ...
    def stop(self):
        self.root.after(3000, self.passar)
        """Muda a variável funcionando para False e interrompe a execução."""
        if self.funcionando == True and self.nome_arquivo is None and self.intervalo_tempo is None:
            self.status.config(text='Programa já parado', bg='yellow')
        elif self.funcionando:
            self.funcionando = False
            self.root.after(1000, self.interromper)
            self.salvar()
            self.teste.clear()
    ...
    def salvar(self):
        ...
        for k, v in self.teste.items():
            ...
            v.save(file_path)
            self.root.after(500, self.passar)
        self.status_salvar.config(text=f'{len(self.teste)} total de prints salvos!', bg='light green')
